...
Path bfs(const Graph &graph, const Vertex &start, std::function<bool(const Vertex &vertex)> goalTest)
{
	...
	while (!queue.empty()) {
		...
		Vertex last;
		if (path.size() == 0) last = start;
		else last = path.back();
		if (goalTest(last)) return path;
...
int main()
{
	...
	Path path1 = dfs(graph, start, [&](Vertex v) { return (v == goal); });
	std::cout << start << std::endl;
	for (auto it = path1.cbegin(); it != path1.cend(); it++)
		std::cout << *it << std::endl;

	//	breadth-first search
	std::cout << "\nBreadth-first search:\n";
	Path path2 = bfs(graph, start, [&](Vertex v) { return (v == goal); });
	std::cout << start << std::endl;
	for (auto it = path2.cbegin(); it != path2.cend(); it++)
		std::cout << *it << std::endl;
